...
import { changeUser } from "../../utils/redux/features/userSlice"
...
const Login = () => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    axios.post(`${process.env.REACT_APP_API_URL}/user/login`, {
      ...
      const { id, name, email } = res.data.verify[0][0]
      dispatch(changeUser({
        id: id,
        name: name,
        email: email,
            ...
            <form onSubmit={handleSubmit} className=" flex flex-col justify-between h-full">
              ...
              <div>
                ...
                <div className=" flex flex-col">
                  ...
                  <input onChange={(e) => setPassword(e.target.value)} type="password" id="PASSWORD" className=" input"/>
                </div>
                ...
                {error &&
                  <div className=" w-full bg-red-300 text-red-600 border border-red-600 rounded p-4 mt-4 text-sm opacity-75">
                    Something went wrong, please try again!
                  </div>
                }
              </div>

              <Button type="submit" color="#eab308" textColor="#a16207" width="100%" text={isLoading ? "Loading" : "Log In"} func={() => console.log("Log in")}/>
            </form>
        </div>
      </main>
    </>
